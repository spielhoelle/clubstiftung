...
/**
 * Class clubstiftung_Related_Posts_Output
 *
...
if( !function_exists( 'clubstiftung_CallRelatedPostsClass' ) ) {
    /**
     *
     ...
     *
     */
    function clubstiftung_CallRelatedPostsClass()
    {
        $display_related_blog_posts = get_theme_mod('clubstiftung_enable_related_blog_posts', 1);

        if ($display_related_blog_posts == 1) {

            // instantiate the class & load everything else
            clubstiftung_Related_Posts_Output::getInstance();
        }
    }
    add_action( 'wp_loaded', 'clubstiftung_CallRelatedPostsClass');
}


if( !class_exists( 'clubstiftung_Related_Posts_Output' ) ) {
	...
	class clubstiftung_Related_Posts_Output {
		...
		protected function __construct() {
			add_action( 'clubstiftung_single_after_content', array( $this, 'output_related_posts' ), 3 );
		}
					...
					$output .= '<div class="row">';
						$output .= '<div class="col-sm-12">';
							$output .= '<div class="related-article-title">';
								$output .= __( 'Related Articles', 'clubstiftung' );
						...
						while( $post_query->have_posts() ) {
							...
							$post_thumbnail = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'clubstiftung-blog-post-related-articles' );
